...
import { KNOWLEDGE_VERSION, MUSE_KNOWLEDGE } from "./muse-knowledge.ts";
...
export async function requestMuse(value: unknown, signal?: AbortSignal) {
  if (new TextEncoder().encode(JSON.stringify(value)).byteLength > MAX_REQUEST_BYTES) throw new Error("request_too_large");
  ...
  let payload: unknown;
  try {
    payload = await requestBrowserResponses(loadBrowserApiConfig(), {
    ...
    }, controller.signal, { blocked: "muse_blocked", failed: "muse_unavailable" });
  } catch (error) {
    if (controller.signal.aborted) throw new Error("muse_timeout");
    throw error;
  } finally { clearTimeout(timeout); signal?.removeEventListener("abort", abort); }

  if (!isRecord(payload) || payload.status === "incomplete") throw new Error("invalid_advice");
  ...
  if (output.some((item) => item.type === "refusal")) throw new Error("muse_blocked");
  ...
  let answer: unknown;
  try { answer = JSON.parse(text); } catch { throw new Error("invalid_advice"); }
  if (!isRecord(answer) || typeof answer.answer !== "string" || !answer.answer.trim() || answer.answer.length > 6000 ||
    ...
    (answer.suggestedStage !== null && !MUSE_STAGES.includes(answer.suggestedStage as MuseStage))) throw new Error("invalid_advice");
  ...
  return { answer: answer.answer.trim(), sources, suggestedStage: answer.suggestedStage, knowledgeVersion: KNOWLEDGE_VERSION };
}
