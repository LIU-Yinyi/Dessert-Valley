...
export async function structureIdea(input: {
  ...
  const response = await fetch("/api/idea-structure", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(input),
  });
  const payload = (await response.json().catch(() => null)) as {
    idea?: unknown; error?: { code?: string };
  } | null;
  if (!response.ok) throw new Error(payload?.error?.code || "idea_failed");
  return ideaCardFields(payload?.idea, input.images);
}
